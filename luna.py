...
USER_AGENT = "Mozilla/5.0 (Luna-Agent/0.1)"
...
def search_web(query):
    errors = []
    try:
        r = duckduckgo_search(query)
        if r:
            return r, errors
        errors.append("DuckDuckGo devolvió 0 resultados")
    except Exception as e:
        errors.append(f"DuckDuckGo falló: {type(e).__name__}: {e}")
    ...
    try:
        ...
        errors.append(f"Wikipedia falló: {type(e).__name__}: {e}")

    return [], errors

def main():
    query = " ".join(sys.argv[1:]).strip() or os.getenv("LUNA_QUERY", "OpenAI artificial intelligence")
    ...
    with open("resultados/ultima_busqueda.json", "w", encoding="utf-8") as f:
